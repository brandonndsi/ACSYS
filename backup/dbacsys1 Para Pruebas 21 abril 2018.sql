...
--
-- Servidor: localhost
-- Tiempo de generación: 21-04-2018 a las 16:56:30
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registrardetalleventadistribuidor` (IN `pre` DOUBLE, IN `can` DOUBLE, IN `sub` DOUBLE, IN `cod` VARCHAR(200), IN `des` DOUBLE, IN `id` INT)  NO SQL
BEGIN
