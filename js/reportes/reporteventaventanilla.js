...
function modalVer(facturanueva) {
    ...
    if (tipo == "Ventanilla") {
        $("#Re_tipoVenta").val("Ventanilla");
        buscardetallesVentanilla(string[5], string[0]);
        abrirModalDetalles();
...
function buscardetallesVentanilla(tol, idv) {
    //alert(idv+", "+idpersona);
    totalAPagar = tol;
    $.post('../../business/reportes/reportesAccion.php', {
        action: 'buscarDetalleVentanilla',
        venta: idv
    }, function (responseText) {
        json = JSON.parse(responseText);
        //console.log(json);
